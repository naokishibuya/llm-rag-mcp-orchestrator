...
async def _agent_node(state: dict, agents: dict) -> Command:
    ...
    updates = {
        ...
        "intent_responses": [result.response],
...
class Orchestrator:
    ...
    async def initialize(self):
        ...
        self._graph = self._build_graph(self._agents, routing)
        logger.info("Orchestrator initialized with %d agents", len(self._agents))

    def _build_graph(self, agents: dict, routing: dict[str, RoutingMeta]):
        moderator = Moderator()
    ...
    async def invoke(
        ...
        model = self._registry.get_chat_model(model_name)
        ...
        initial_state = {
            ...
            # Execution (initialized for reducers)
            "agent_response": "",
            "agent_success": True,
            "intent_responses": [],
            "tool_results": {},
